...
if (!Config.STANDALONE_MODE) {
    (async () => {
        try {
            const dyp = await import("../../../api/dyp.js");
            DynamicProperties = dyp.DynamicProperties;
        } catch (e) {
            Logger.warn("DynamicProperties not available (Make a Country not found)");
        }
    })();
}

function safeSendMessage(sender, msg) {
    if (!sender) return;
    try {
...
export function scanAndCompressChunk(dimension, cx, cz) {
    const palette = [];
    const paletteMap = new Map();
    const s_ids = [];
    const s_ys = [];

    const getPaletteId = (typeId) => {
...
export async function checkExistingChunks(chunkCoords) {
    ...
    try {
        ...
        if (res.status === 200) {
            ...
            const existingSet = new Set(result.existing.map(c => `${c.cx},${c.cz}`));
            return chunkCoords.filter(c => !existingSet.has(`${c.cx},${c.cz}`));
        }
    } catch (e) {
...
export async function sendChunkDataBatch(chunksData) {
    if (!chunksData || chunksData.length === 0) return;
    await postJson('/api/map/update', { data: chunksData });
}

export async function sendPlayerData(playersData) {
    await postJson('/players', { data: playersData });
}

export async function syncAllCountryData() {
    if (Config.STANDALONE_MODE || !DynamicProperties) {
        Logger.info("Country sync skipped (STANDALONE_MODE or DynamicProperties unavailable)");
        return 0;
    }
    ...
    try {
        ...
        for (const key of countryIds) {
            ...
            const membersName = (countryData.members || []).map(memberId => {
            ...
            const allianceCountryName = (countryData.alliance || []).map(aid => {
            ...
            const friendlyCountryName = (countryData.friendly || []).map(fid => {
            ...
            const hostilityCountryName = (countryData.hostility || []).map(hid => {
            ...
            countriesData.push({
                ...
                members: membersName.filter(n => n !== (ownerData?.name)).join(' , '),
