...
system.beforeEvents.startup.subscribe(ev => {
    try {
        ...
        Logger.info("Registered command: /wmap");
    } catch (e) {
...
world.beforeEvents.chatSend.subscribe(async (ev) => {
    ...
    if (!ev.message.startsWith("!wmap ")) return;
    ev.cancel = true;
 ...
 * メッセージ送信の安全化ヘルパー
 */
function safeSendMessage(executor, msg) {
    if (!executor) return;
    try {
 ...
 * コマンド処理の共通ロジック
 */
async function executeMapLogic(origin, args) {
    const sender = origin.sourceEntity;

    const consoleExecutor = {
        name: "Server",
        id: "server",
        sendMessage: (msg) => Logger.info(msg),
        hasTag: () => true,
        isOp: true,
        isValid: true
    };
    ...
    if (!action) {
        safeSendMessage(executor, "§cUsage: /wmap <render|repair|stop|resume|reset|sync>");
        return;
    }

    const parseBounds = () => {
        const numParams = params.map(p => parseInt(p)).filter(n => !isNaN(n));
...
async function postJsonPartial(endpoint, data) {
...
function showResult(executor, stats, stopped) {
...
async function startRender(executor, bounds, resumeCursor = null, mode = "force") {
    isRendering = true;
    stopRequested = false;

    const sendMsg = (msg) => safeSendMessage(executor, msg);
